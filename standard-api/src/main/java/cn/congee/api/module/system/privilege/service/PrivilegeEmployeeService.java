...
import cn.congee.api.module.system.systemconfig.domain.dto.SystemConfigDTO;
...
public class PrivilegeEmployeeService {
    ...
    public Boolean isSuperman(Long employeeId) {
        ...
        if (systemConfig == null) {
            throw new StandardBusinessException("缺少系统配置项[" + SystemConfigEnum.Key.EMPLOYEE_SUPERMAN.name() + "]");
        }

        List<Long> superManIdsList = StandardStringUtil.splitConverToLongList(systemConfig.getConfigValue(), ",");
    ...
    public Map<String, List<String>> updateCachePrivilege(Long employeeId, List<PrivilegeEntity> privilegeEntities) {
        ...
        if (CollectionUtils.isNotEmpty(privilegeEntities)) {
            List<List<String>> setList =
                    privilegeEntities.stream().filter(e -> e.getType().equals(PrivilegeTypeEnum.POINTS.getValue())).map(PrivilegeEntity::getUrl).collect(Collectors.toList()).stream().map(e -> StandardStringUtil.splitConvertToList(e, ",")).collect(Collectors.toList());
            setList.forEach(privilegeList::addAll);
        }
        privilegeList.forEach(item -> {
            List<String> path = StandardStringUtil.splitConvertToList(item, "\\.");
