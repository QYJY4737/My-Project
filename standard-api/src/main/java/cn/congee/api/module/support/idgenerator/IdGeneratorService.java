...
import cn.congee.api.module.support.idgenerator.constant.IdGeneratorEnum;
...
public class IdGeneratorService {
    ...
    private IdGeneratorPOJO validateParams(IdGeneratorEnum idGeneratorEnum, int stepLength) {
        if (stepLength < 1) {
            throw new StandardBusinessException("IdGenerator， step过短" + stepLength);
        }

        IdGeneratorPOJO idGeneratorPOJO = this.idGeneratorMap.get(idGeneratorEnum.getId());
        if (idGeneratorPOJO == null) {
            throw new StandardBusinessException("IdGenerator， id 不存在" + idGeneratorEnum);
        }
