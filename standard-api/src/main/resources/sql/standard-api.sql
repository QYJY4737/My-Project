...
-- --------------------------------------------------------
-- 导出 glonk 的数据库结构
DROP DATABASE IF EXISTS 'glonk';
CREATE DATABASE IF NOT EXISTS 'glonk';
USE glonk;

-- 导出  表 glonk.t_department 结构
...
-- 正在导出表  glonk.t_department 的数据：~4 rows (大约)
...
-- 正在导出表  glonk.t_email 的数据：~56 rows (大约)
...
-- 正在导出表  glonk.t_employee 的数据：~28 rows (大约)
...
-- 正在导出表  glonk.t_file 的数据：~23 rows (大约)
...
-- 导出  表 glonk.t_heart_beat_record 结构
...
-- 正在导出表  glonk.t_heart_beat_record 的数据：~2 rows (大约)
...
-- 正在导出表  glonk.t_id_generator 的数据：~2 rows (大约)
...
-- 导出  表 glonk.t_id_generator_record 结构
...
-- 正在导出表  glonk.t_id_generator_record 的数据：~5 rows (大约)
...
-- 正在导出表  glonk.t_notice 的数据：~14 rows (大约)
...
-- 导出  表 glonk.t_notice_receive_record 结构
...
-- 正在导出表  glonk.t_notice_receive_record 的数据：~27 rows (大约)
...
-- 导出  表 glonk.t_order_operate_log 结构
...
-- 正在导出表  glonk.t_peony 的数据：~3 rows (大约)
...
-- 正在导出表  glonk.t_position 的数据：~13 rows (大约)
...
-- 导出  表 glonk.t_position_relation 结构
...
-- 正在导出表  glonk.t_position_relation 的数据：~27 rows (大约)
...
-- 正在导出表  glonk.t_privilege 的数据：~103 rows (大约)
...
-- 正在导出表  glonk.t_quartz_task 的数据：~5 rows (大约)
...
-- 导出  表 glonk.t_quartz_task_log 结构
...
-- 正在导出表  glonk.t_quartz_task_log 的数据：~11 rows (大约)
...
-- 正在导出表  glonk.t_reload_item 的数据：~0 rows (大约)
...
-- 正在导出表  glonk.t_reload_result 的数据：~127 rows (大约)
...
-- 正在导出表  glonk.t_role 的数据：~14 rows (大约)
...
-- 导出  表 glonk.t_role_data_scope 结构
...
-- 正在导出表  glonk.t_role_data_scope 的数据：~4 rows (大约)
...
-- 正在导出表  glonk.t_role_employee 的数据：~25 rows (大约)
...
-- 导出  表 glonk.t_role_privilege 结构
...
-- 正在导出表  glonk.t_role_privilege 的数据：~322 rows (大约)
...
-- 正在导出表  glonk.t_system_config 的数据：~8 rows (大约)
...
-- 导出  表 glonk.t_user_login_log 结构
...
-- 正在导出表  glonk.t_user_login_log 的数据：~122 rows (大约)
...
-- 导出  表 glonk.t_user_operate_log 结构
...
-- 正在导出表  glonk.t_user_operate_log 的数据：~32 rows (大约)
